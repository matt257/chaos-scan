...
export async function POST(request: NextRequest) {
  try {
      ...
      for (const fact of normalizedFacts) {
        const savedFact = await prisma.fact.create({
          data: {
            ...
            entityRaw: fact.entity_raw,
            entityCanonical: fact.entity_canonical,
    ...
    // Generate summary with prune stats and scan mode
    const summary = await generateSummary(
      analysisResult.issues,
      analysisResult.pruneStats,
      { scanMode: analysisResult.scanMode }
    );
